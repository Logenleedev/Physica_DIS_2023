  ...
  if (drop) {
    ...
    for (int i = 0; i< nCubes; ++i) {
      ...
      if (cubes[i].isLost==false) {
        ...
        //println("----------------");
        if (dist(cubes[i].x, cubes[i].y, mParticle.position().x, mParticle.position().y) > 20 && cubes[i].speedY < 0 && cubes[i].pre_speedY[1] > 0) {
          mParticle.position().set(cubes[i].x, cubes[i].y);
          mParticle.velocity().set(mParticle.velocity().x, -1*(mParticle.velocity().y));

          cubes[i].state += 1;
        }
        //test for diameter bouncing
        println("pre", cubes[i].pre_deg);
        println(cubes[i].deg);

        if (dist(cubes[i].x, cubes[i].y, mParticle.position().x, mParticle.position().y) > 20 && sqrt(pow(cubes[i].speedX, 2)+pow(cubes[i].speedY, 2))<10 ) {
          mParticle.position().set(cubes[i].x, cubes[i].y);
          if (cubes[i].deg > cubes[i].pre_deg) {
            mParticle.velocity().set(25, -1*(mParticle.velocity().y));
          }
          //-16*cubes[i].preSpeedX
          if (cubes[i].deg < cubes[i].pre_deg) {
            mParticle.velocity().set(-25, -1*(mParticle.velocity().y));
          }
          cubes[i].state += 1;
