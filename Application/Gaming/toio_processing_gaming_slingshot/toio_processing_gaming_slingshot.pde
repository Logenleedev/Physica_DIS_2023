...
void draw() {
  ...
  if (drop) {
    ...
    if (checkbox3.getArrayValue()[2] == 1 ) {
      ...
      String a = "Force is: " + calculateSpringForce(mSpring.strength(), mSpring.currentLength() - mSpring.restlength())+"N";
    ...
    //drawArrow(200, 150, 200, 150+gravity_num, 0);
    if (checkbox2.getArrayValue()[3] == 1 ) {
      ...
      if (cubes[0].isLost==false) {
        ...
        offscreen.stroke(strokeCol);
        offscreen.translate(cubes[0].x - projection_correction, cubes[0].y - projection_correction);
        offscreen.rotate(cubes[0].deg * PI/180);
    ...
    if ((cubes[1].x >= 902 & cubes[1].x <= 938 )& (cubes[1].y >= 260 & cubes[1].y <= 456 )) {
      gravity_num = (196.0/2-(cubes[1].y - 260))*2;
