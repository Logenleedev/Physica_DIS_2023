...
void grab_release() {
  ...
  for (int i = 0; i< nCubes; ++i) {
    ...
    if (checkbox3.getArrayValue()[1] == 1 && cubes[i].state >= 2) {
    ...
    if (checkbox3.getArrayValue()[3] == 1) {
      // draw mParticle vector
      offscreen.pushMatrix();
      offscreen.translate(cubes[0].x, cubes[0].y);
      offscreen.stroke(191, 201, 202);
      if (cubes[0].ave_speedY != 0 && cubes[0].ave_speedX!=0) {

        drawArrow(0, 0, cubes[0].ave_speedX, 0, 0);
      }
      offscreen.popMatrix();
    }

    if (checkbox3.getArrayValue()[4] == 1) {
      // draw mParticle vector
      offscreen.pushMatrix();
      offscreen.translate(cubes[0].x, cubes[0].y);
      offscreen.stroke(17, 198, 234 );
      if (cubes[0].ave_speedY != 0 && cubes[0].ave_speedX!=0) {
        drawArrow(0, 0, 0, cubes[0].ave_speedY, 0);
      }
      offscreen.popMatrix();
    }
